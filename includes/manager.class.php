	...
	public function generateRoomListHtml(){
		...
		while($row = $result->fetch_assoc()){
			...
			$rthtml .= '<tr>
						  ...
						  <td>'.$row['NoOfRoom'].'</td>
						  <td class="center" align="right"></td></tr>';//edited. check original class
		}
		$rthtml .= '</tbody>';
		return $rthtml;
	}
...
<td>'.$bsiCore->currency_symbol().'<input type="text" name="sat" id="sat" class="required number" value="'.$row['sat'].'" size="4" /></td>
	...
	public function changePassword(){
		...
		$result  = $mysqli->query("select * from bsi_manager where pass=\"" . md5($oldpass) . "\" and id=".$adminid);
		if($result->num_rows){
			$mysqli->query("update bsi_manager set pass='".md5($newpass)."' where id=".$adminid);
