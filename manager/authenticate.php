...
//	header("location:index.php");
//	exit;
//}else{ I REMOVED THE CAPTCHA
	
				$username = $mysqli->real_escape_string($_POST['username']);
				$password = $_POST['password'];
				$result   = $mysqli->query("select * from bsi_manager where pass='".md5($password)."' and  username='".$username."'");
				...
				if($count){
					...
					$mysqli->query("update bsi_manager set last_login=CURRENT_TIMESTAMP where id=".$row['id']);
	...
	case 2: if(isset($_POST['emailid'])){
				...
				$result  = $mysqli->query("select * from bsi_manager where email='".$emailid."'");
				if($result->num_rows){
					...
					$mysqli->query("update bsi_manager set pass='".md5($temp_password)."' where id=".$row['id']);
					$subject =  "Admin Panel : Your password has been reset";
                    $body    =  "Hi,<br><br>" .
